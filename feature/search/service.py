...
async def search_novels_by_text(db: AsyncSession, query: str, limit: int | None = None) -> list[Novel]:
    title_stmt = select(Novel).where(Novel.novel_title.ilike(f"%{query}%"), Novel.novel_isprivate == False)
    ...
    author_stmt = select(Novel).where(Novel.novel_author.ilike(f"%{query}%"), Novel.novel_isprivate == False)
    if limit is not None:
